...
🔴 **The single most important limitation, stated where it cannot be missed.** The panel's negatives split **178 train / 118 calibrate / 0 test**. There is no held-out test set, so every false-positive rate this project has published was measured on negatives the pipeline had already seen. Measured out of sample against an 8,259-protein benign pool over 200 seeds, replicated on two model arms, a nominal 5% budget really costs about **7.9%**, and about **10%** once the pool is collapsed to distinct names. That is the project's own criterion 1 and it scores a fail on it. Section 2.6.1 of [`docs/MECHANISM_GENERALIZATION.md`](docs/MECHANISM_GENERALIZATION.md) decomposes the gap into estimator, distribution shift and name redundancy.

Two design documents sit alongside the results. [`docs/DETECTOR_CRITERIA.md`](docs/DETECTOR_CRITERIA.md) states eighteen criteria for evaluating a hazard detector, each anchored to a place where this panel's own probe fell short, and scores this project against them (three fails, three partials, one mixed). The newest is criterion 18: the benign reference set is an attack surface, and 500 genuine Swiss-Prot proteins labelled benign can cut one hazard class from 63.5% to 16.7% while the false-positive rate improves. [`docs/MUTATION_EXTENSION_PREREGISTRATION.md`](docs/MUTATION_EXTENSION_PREREGISTRATION.md) is a frozen design for extending the metrics to point mutations, written before any run, including the reason the obvious version of that experiment does not work.
...
**Mean FSPE ratio: 0.64. Protein-level test (n = 14): 12/14 below 1.0, sign test p = 0.0065, permutation p = 0.0001.** A residue-pooled Mann–Whitney over 74 functional vs 300 background residues gives p = 4.5 × 10⁻¹⁰, but it treats residues within a protein as independent and is descriptive only.

> 🔴 **These numbers changed on 2026-09-22, and they got weaker.** SEB (`P01552`) has been **excluded** from the protein-level test, taking it from 13/15 at p = 0.0037 to **12/14 at p = 0.0065**. Two independent reasons. Its published coordinate offset of 0 is *disproven*: the entry has a signal peptide at 1-27 and two of its nine positions fall inside that cleaved peptide while being annotated "MHC-II binding interface", which a secreted superantigen cannot do. The only admissible frame, +27, places just 1 of 9 annotated residue identities correctly and so fails the acceptance rule every other corrected entry had to meet, leaving no usable annotation for this protein. Independently, SEB was **already** excluded from FSI because a superantigen has no discrete catalytic site, and that objection applies here unchanged, so this also closes a long-standing inconsistency between the two metrics.
>
> ⚠️ The two tests move in opposite directions and both are reported. The sign test **weakens** (0.0037 → 0.0065) because a success is removed. The permutation test **strengthens** (0.0002 → 0.0001) because the removed ratio, 0.956, was the closest to 1.0 among the successes, so the mean log ratio becomes more negative (−1.850 → −1.979). Quoting only the second would be cherry-picking. Full treatment in [`docs/EVALUATION_REPORT.md`](docs/EVALUATION_REPORT.md) and entries nineteen and twenty of [`docs/DATA_CORRECTIONS.md`](docs/DATA_CORRECTIONS.md).

FSPE provides directional evidence (6/8 proteins show ratio < 1, mean 0.64), with Tetanus LC and BoNT-A reaching per-protein significance (both p < 0.0001, r = 1.00) and Cholera and Streptolysin O nominally significant (p = 0.014 and 0.025). Individual Mann–Whitney tests are structurally underpowered for proteins with few annotated catalytic sites; the better-powered test is run at the **protein level** (p = 0.0065 sign test, p = 0.0001 permutation, n = 14 after the SEB exclusion above).
