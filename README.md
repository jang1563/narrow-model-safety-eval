...
> 🔴 **Do not quote that headline without this.** One of the thirteen, SEB (`P01552`, ratio 0.956), is scored at a coordinate offset that UniProt **rules out**: the entry has a signal peptide at 1-27, and two of its nine annotated positions fall inside that cleaved peptide while being annotated "MHC-II binding interface". In the only admissible frame, +27, its ratio becomes 1.0417 and crosses 1.0, making the panel **12/15 at p = 0.0176**. Excluding the entry instead gives **12/14 at p = 0.0065**. The published figures stand pending a deliberate decision, but the honest short form is "13/15 at p = 0.0037, and 12/14 at p = 0.0065 with the annotation-flagged entries removed". Full treatment in [`docs/EVALUATION_REPORT.md`](docs/EVALUATION_REPORT.md) and the nineteenth entry of [`docs/DATA_CORRECTIONS.md`](docs/DATA_CORRECTIONS.md).
